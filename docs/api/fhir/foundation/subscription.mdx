...
description: Notification request when content changes. FHIR R5 resource, served by the Ovok FHIR API.
---
...
Notification request when content changes.

## Endpoints

The standard FHIR R5 interactions for `Subscription` are mounted under your
...
| Vread        | `GET`    | `/Subscription/[id]/_history/[vid]` |
| Update       | `PUT`    | `/Subscription/[id]` |
| Patch        | `PATCH`  | `/Subscription/[id]` |
| Delete       | `DELETE` | `/Subscription/[id]` |
| Create       | `POST`   | `/Subscription` |
| Search       | `GET`    | `/Subscription?...` |
| History      | `GET`    | `/Subscription/[id]/_history` |
| Type-history | `GET`    | `/Subscription/_history` |
...
- Search parameters: [`Subscription` search params](https://hl7.org/fhir/R5/subscription.html#search)
