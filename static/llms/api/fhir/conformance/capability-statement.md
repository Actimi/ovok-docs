...
description: "A Capability Statement documents a set of capabilities (behaviors) of a FHIR Server or Client for a particular version of FHIR that may be used as a statement o"
---
...
`CapabilityStatement` supports the parameters below.
...
| `format` | `token` | formats supported (xml | json | ttl | mime type) |
...
| `security-service` | `token` | OAuth | SMART-on-FHIR | NTLM | Basic | Kerberos | Certificates |
...
- Element bindings & profiles: [`CapabilityStatement` profile](https://hl7.org/fhir/R5/capabilitystatement-definitions.html)
