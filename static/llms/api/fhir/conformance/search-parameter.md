...
<ApiBase surface="fhir" path="/SearchParameter" inline={false} />
...
| `type` | `code` | `1..1` | number | date | string | token | reference | composite | quantity | uri | special |
| `expression` | `string` | `0..1` | FHIRPath expression that extracts the values |
| `processingMode` | `code` | `0..1` | normal | phonetic | other |
...
| `comparator` | `code` | `0..*` | eq | ne | gt | lt | ge | le | sa | eb | ap |
| `modifier` | `code` | `0..*` | missing | exact | contains | not | text | in | not-in | below | above | type | identifier | of-type | code-text | text-advanced | iterate |
...
`SearchParameter` supports the parameters below.
...
| `type` | `token` | number | date | string | token | reference | composite | quantity | uri | special |
| `url` | `uri` | The uri that identifies the activity definition |
| `version` | `token` | The business version of the activity definition |

## Reference

- Official FHIR R5 spec: [`SearchParameter`](https://hl7.org/fhir/R5/searchparameter.html)
- Element bindings & profiles: [`SearchParameter` profile](https://hl7.org/fhir/R5/searchparameter-definitions.html)
